...
A call for each season can be made, this function cam be automatize to do it for all the period but this is only an example. 

The last season tensor is unable to get because there is not  enought data. 

s
Here the band names has to be rename using a python script in order the daily stimation could be done. 
...
```
A file to move the daily file to the corresponding year si made call `movev_pred_m3.sh`  this file should move the data using the prefix, in this example is no useful. 