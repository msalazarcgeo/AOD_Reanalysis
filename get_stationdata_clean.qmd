...
This notebook is build only to show how to generate a data frame that was used to obtain the results of the article.

Since it has to be store in a `RDA` I will do it on *R*. Since all the data has been downloaded in a specific path
...
Open all the files and to merge into a single data frame
...
Originally this is done year by year here is an example that can work for the single year
...
With all the data available create the data frame in columns for mean for the day if a new value is needed here is where the changes should be made.
